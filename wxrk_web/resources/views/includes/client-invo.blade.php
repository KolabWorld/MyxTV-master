...
<div class="invoice-box" style="border-radius:0px !important;padding:0px !important;border:0px !important;margin:0px !important;">
	<table cellpadding="0" cellspacing="0" width="100%" border="0" style="border-radius:0px !important;padding:0px !important;border:0px !important;margin:0px !important;">
				...
				<table style="border-radius:0px !important;padding:0px !important;border:0px !important;margin:0px !important;">
					<tr>
						<td class="title" style="border-radius:0px !important;padding:0px !important;border:0px !important;margin:0px !important;">
							<img src="{{$logo}}" class="logo" style="max-width:120px;max-height:120px;">
						</td>
						<td style="border-radius:0px !important;padding:10px 0px 0px 0px !important;border:0px !important;margin:0px !important;text-align:right;">
							<address class="small-text">
								<span class="headi" style="font-size: 14px;font-weight:bold;">
				...
				<table>
					<tr>
						<td class="small-text" style="font-size:12px;text-align:left;">
							<p class="lead marginbottom payment-info" style="font-size:12px;font-weight:bold;">
								Invoiced To<br>
							</p>
						...
						<td style="font-size:12px;text-align:right;word-wrap: break-word;width:300px;">
							...
							<address class="small-text">
								{{$configurationData ? $configurationData->name : ''}}<br>
								{{$configurationData && $configurationData->address ? $configurationData->address->line_1 : ''}}<br>
								Ph.: {{$configurationData ? $configurationData->mobile : ''}}<br>
								Email ID: {{$configurationData ? $configurationData->email : ''}}
							</address>
						</td>
					</tr>
				</table>
		...
		@foreach($invoice->items as $key => $item)
			...
			@if($item->item_sequence == $old_item_sequence)
				<tr class="details">
					...
					<td style="padding:0px 10px 0px 20px;height:10px;text-align: right;font-size:11px;">
			...
			@else
				<tr class="details">
					...
					<td style="text-align: right;font-size:11px !important;">
						<span style="float:right;">
							<address class="small-text" style="font-size:11px;">{{$invoice->currency}} @numberFormat($item->sub_total)</address>
						</span>
					</td>
			...
			<tr class="heading">
				...
				<td style="text-align:right;font-size:11px;">
					<address class="small-text" style="font-size:11px;">
						Sub Total : {{$invoice->currency}} @currencyPDF($invoice->sub_total)
					</address>
				</td>
			</tr>
		...
		@if($invoice->total_tax > 0)
			...
			@if($tax2)
				<tr class="heading">
					...
					<td style="text-align:right;font-size:11px;">
						<address class="small-text"> 
							{{$tax2}} ({{$taxPerc2}}%): {{$invoice->currency}} @currencyPDF($taxValue)
						</address>
					</td>
				</tr>
			@endif
		...
		<tr class="heading">
			...
			<td style="text-align:right;font-size:11px;">
				<address class="small-text"> 
					Total Amount : {{$invoice->currency}} @currencyPDF($invoice->invoice_total)
				</address>
			</td>
		</tr>
	</table>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tg">
		<tr>
			<td width="49%">
				<address class="small-text"  style="font-size:11px;">
					<b>Pan Number. -</b> {{$configurationData ? $configurationData->pan_number : ''}}<br />
					<b>GST No. -</b> {{$configurationData ? $configurationData->gstin_number : ''}}<br />
					<b>Account No. -</b> {{$configurationData ? $configurationData->account_number : ''}}<br />
					<b>IFSC Code -</b> {{$configurationData ? $configurationData->ifsc_code : ''}}<br />
				</address>
			</td>
			<td width="2%">&nbsp;</td>
			<td width="12%">&nbsp;</td>
        ...
        <!-- <tr>
        	<td height="141" colspan="4" valign="top" style="font-size:11px; padding:0px !important;">
				<address class="small-text" style="font-size:11px;padding:0px !important;">
					<strong  style="font-size:11px;padding:0px !important;">Terms and Conditions</strong>
					<br/>
	...
	<div class="footer" style="font-size:11px;">
		<p>{{$configurationData ? $configurationData->footer_description : ''}}</p>
	</div>
	<!-- <div class="footer" style="font-size:11px;">
		{{$configurationData ? $configurationData->name : ''}}
